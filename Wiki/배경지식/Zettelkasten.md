...
```
Fleeting (Daily/, Notes/Inbox/)  →  Literature (Sources/)  →  Permanent (Wiki/)
    내가 쓴다                         Q&A 검증 후 정제        LLM이 정제한다
```

전통 Zettelkasten에서는 사람이 Permanent note를 직접 쓰지만, LLM Wiki에서는 **LLM이 이 과정을 대신**한다. 단, `Notes/` 직접 영역에 쓰는 원칙·생각·evergreen은 사용자가 계속 유지·수정하는 **가변 영구 노트**로 별도 운영된다.
